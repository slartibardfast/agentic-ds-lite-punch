...
the 2024 model ("the CPE routes exactly one /64 downstream", per the
...
- `wan4o6` (router-side ds-lite B4 toward `aftr01.upc.ie` /
  ...
  This held from the hub-LAN /64, from fresh IIDs, from a hand-carved /57
  subnet, and across a CPE restart. The Hub 6 (LG-RDK, replaced the Sagemcom)
  runs ds-lite itself and the AFTR accepts only the hub's softwire.
...
admin page all agree on `aftr01.upc.ie`, resolving to `2001:730:2000:2::353`;
the refusal is per-subscriber B4 binding, not a stale endpoint).
