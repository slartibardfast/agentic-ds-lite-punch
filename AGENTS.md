...
This section is written by the adopter and is not part of the template spine.
An upgrade re-applies the spine files verbatim and leaves this section alone.

- **Identity.** This host is `agentic-ds-lite-punch`, origin
  `https://github.com/slartibardfast/agentic-ds-lite-punch.git`. It externalizes
  the thought about the ds-lite-punch software; the software itself is the hosted
...
- **The investigation record.** `plan/0004-ds-lite-punch/INVESTIGATION.md` was the
  repo-root `DSLITE.md`, moved into the milestone room and rewritten to zero prose
  tropes during adoption. It holds the possibility-space exploration of the
  Virgin Media AFTR that fed call/0011 and plan/0004.
- **Legacy path references.** Pre-adoption documents cite the old monorepo layout:
  `tools/ds-lite-punch/` means the `ds-lite-punch` component (see `.host-software`);
  `MEMORY.md` entries named there resolve in this host's `MEMORY.md`.
- **Reconciliation deltas.** network ground truth recorded during development
  lives in `MEMORY.md`; where a plan document conflicts with a delta, the delta
  wins (see plan/0004 IMPLEMENTATION.md "Reconciliation deltas").
