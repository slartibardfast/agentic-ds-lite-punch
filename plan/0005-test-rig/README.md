...
in `tools/`; nothing is applied to the router until the review checklist
below is signed. The rig closes the A1, A3, C6, and G8 acceptance paths of
plan/0004 that need a second host, and hosts the TCP idle-lifetime
measurement. Tooling migrates into the ds-lite-punch component at crate
migration (call/0012).
...
`tools/router/run-soak.py` drives, on the router: a 10 min baseline, a
...
count port reuse. `tools/router/run-c3.py` measures the TCP idle-lifetime:
...
RST is the dead signal). The full data dictionary is in the tools'
invocation headers.
...
3. Apply tools/router/nft-bringup.sh; verify the probe egress reveals the
   public source 84.203.115.61 (STUN from dslp-probe).
4. Retarget the relay with tools/router/relay-retarget.sh to-sink.
5. Run the campaign; pull the raw data; analyze; commit the dated results
   record and a MEMORY.md entry.

## Results home

Raw captures stay local under `tools/results/` (gitignored). The derived
