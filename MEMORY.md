...
- Peer distances: measured RTT (median) to the match peers 11 to 18 ms
  ...
  route: 51.36.255.29 (the main host, 90% of game traffic) registers to
  Zain Saudi Arabia yet answers at 18 ms from the UK; 72.84.102.221
  registers to Verizon US yet answers at 17 ms. Use measured RTT, never
  the registry country, for distance claims.
...
- Organic NAT Type 2 measured under capture: PSN NAT servers
  52.40.62.54/55 on UDP 3478/3479 echo the console's ephemeral source
  ports 53004/53010; the console SSDP M-SEARCHed 239.255.255.250:1900
  with no IGD answering, so Type 2 came from hole punching alone
  (phase-E relevant). NOT relay-attributed: the relay still targets the
  test sink, no 192.168.21.138 fold entries, and ephemeral probe ports
  cannot ride a single-target 3478 pin. The organic Type 2 does not sign
  plan/0006's #a2-verdict.
